...
const {MONGO_URI, DB_NAME, MAIN_SERVER_URL, TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER} = process.env;
const client = twilio(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN);

const NGROK_URL = 'https://dbf4-72-238-211-48.ngrok-free.app'; // Update this with your actual ngrok URL
...
async function initiateOutboundCall(phoneNumber, campaignData) {
    ...
    try {
        ...
        const call = await client.calls.create({
            url: `${MAIN_SERVER_URL}/twiml?message=${encodedMessage}`,
            to: phoneNumber,
            from: TWILIO_PHONE_NUMBER,
            record: true,
            recordingStatusCallback: `${MAIN_SERVER_URL}/recording-status?campaignId=${campaignData._id}&phoneNumber=${encodeURIComponent(phoneNumber)}`,
